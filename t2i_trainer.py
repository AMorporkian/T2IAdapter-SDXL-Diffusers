804708# @title Main code segment. Extremely long! Make sure you run it though.
import argparse
import functools
import gc
import logging
...
def main(args):
    ...
    if args.t2i_adapter_model_name_or_path:
        logger.info("Loading existing t2i_adapter weights")
        t2i = T2IAdapter.from_pretrained(args.t2i_adapter_model_name_or_path)
    else:
        logger.info("Initializing t2i_adapter weights from unet")
        t2i = T2IAdapter(channels=[320,640,1280], in_channels=1)
...
if __name__ == "__main__":
    ...
    args = Namespace(
        ...
        push_to_hub=True,
        hub_token=None,
        hub_model_id="t2i-adapter-sdxl-segmentation",
        logging_dir="logs",
        allow_tf32=True,
        report_to="wandb",
        ...
        dataset_name="SargeZT/coco-stuff-captioned",
        ...
        tracker_project_name="sd_xl_train_t2i",
    )
    # print(args)
    main(args)
